...
outfitting = {
    ...
    selectedShip: CA,
    mask: [],
    defaultInfoText: ["Select a ship, then choose a race to","enter the game.","","Click 'HELP' below for controls.","","New players should try a Cruiser first."],
    ...
    init: function(canvas) {
        this.oCanvas = canvas || new Canvas(document.body, this.canvasWidth, this.canvasHeight, {fill:"black"});
        ...
        this.raceButtons.push(this.makeRaceButton("ROM", "Romulans", ROM, bufferPx, bufferPx, "#0F0", "#030"));
        this.raceButtons.push(this.makeRaceButton("KLI", "Klingons", KLI, rightAlignPx, bufferPx, "#F00", "#300"));
        this.raceButtons.push(this.makeRaceButton("FED", "Federation", FED, bufferPx, bottomAlignPx, "#FF0", "#330"));
        this.raceButtons.push(this.makeRaceButton("ORI", "Orions", ORI, rightAlignPx, bottomAlignPx, "#33F", "#003"));
        ...
        this.shipButtons[SC] = this.makeShipButton("Scout", SC, justLeftPx, topRowPx, "#0FF", "#033", ["Fast and light, the Scout excels at","dodging enemy fire and rushing behind","enemy lines to bomb planets.","","It has weak phasers, but fast torpedos,","making it fit for long-range fighting."]);
        this.shipButtons[DD] = this.makeShipButton("Destroyer", DD, centerPx, topRowPx, "#0FF", "#033", ["The Destroyer is a challenging ship to", "command effectively, but is noted for","its superior cloaking abilites.","","Useful for suicide runs on Starbases."]);
        this.shipButtons[CA] = this.makeShipButton("Cruiser", CA, justLeftPx, bottomRowPx, "#0FF", "#033", ["A well-balanced ship, favored widely","by captains of all skill levels.","","Highly recommended for new players."]);
        this.shipButtons[BB] = this.makeShipButton("Battleship", BB, centerPx, bottomRowPx, "#0FF", "#033", ["Slow and powerful, Battleships can take","a beating, but their powerful weapons","use up fuel quickly.","","Excellent for planet defense."]);
        this.shipButtons[AS] = this.makeShipButton("Assult", AS, justRightPx, topRowPx, "#0FF", "#033", ["With proper support, the Assult Ship can", "capture planets quite swiftly. It holds","more armies than any other ship and it","is an unmatched planet bomber.","","It also withstands damage very well."]);
        this.shipButtons[SB] = this.makeShipButton("Starbase", SB, justRightPx, bottomRowPx, "#0FF", "#033", ["Available only to the most experienced", "players, a Starbase can usually be", "destroyed only by a coordinated effort","(called 'ogging')."]);
        this.shipButtons[SB].opacity = 0.3;
        this.otherElems.push(new Line(justRightPx-5,topRowPx+10,justRightPx-5,bottomRowPx+this.shipButtonDim-10, {stroke:"#0CC", strokeWidth:2}));
       
        this.infoBox = new Rectangle(420, 120, {x:centerPx - 210 + this.shipButtonDim/2, y:bottomRowPx+this.shipButtonDim+20, rx:10, ry:10, strokeWidth:2, stroke: "#5FF", fill:"#033"});
