...
NetrekConnection = function(webhost, webport, callback) {
    ...
    this.readMessages = function() {
        // if there are no messages, indicate that we are done reading and try again later
        if(this.buffer.length == 0) {
            this.reading = false;
            setTimeout(function(that){ return function() { if(!this.reading) { console.log("looping from timeout"); that.readMessages(); } } }(this), 200);
            return;
        }
        ...
        // only read if there is a full message on the buffer; otherwise wait
...
serverPackets = [
  ...
  { // SP_PLAYER_INFO
    ...
    handler: function(data) {
        var uvars = packer.unpack(this.format, data);
        var ignored = uvars.next(), pnum = uvars.next(), shiptype = uvars.next(), team = uvars.next();
        if(net_logging) console.log("SP_PLAYER_INFO pnum=",pnum,"shiptype=",shiptype,"team=",team_decode(team));
        if(world.ships[pnum] == undefined) world.addShip(pnum, new Ship({ img: imageLib.images[0][shiptype], galImg: imageLib.images[0][shiptype] }));
    }
  },
  { // SP_KILLS
    code: 3,
...
IND=0x0, FED=0x1, ROM=0x2, KLI=0x4, ORI=0x8;
